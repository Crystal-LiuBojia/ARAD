...
def train():
    ...
    for batch in tqdm(seq_batches(train_data, batch_size=BATCH), desc="Training"): #for batch in train_data.seq_batches(batch_size=BATCH):
...
def test_new(inference_data):
    ...
    for batch in tqdm(seq_batches(inference_data, batch_size=BATCH), desc="Testing"):# for batch in inference_data.seq_batches(batch_size=BATCH):
        batch = batch  #.to(device)
        src, pos_dst, t, msg = batch.src, batch.dst, batch.t, batch.msg
        neg_dst = torch.randint(min_dst_idx, max_dst_idx + 1, (src.size(0),),
                                dtype=torch.long, device=device)
