...
def ground_truth_label():
    labels = {}
    filelist = os.listdir(f"{artifact_dir}/graph_4_6_10min")
    for f in filelist:
        labels[f] = 0
    filelist = os.listdir(f"{artifact_dir}/graph_4_7_10min")
    for f in filelist:
        labels[f] = 0
    ...
    # ]
    attack_list = [
        #'2018-04-06 11:14:26.806182605~2018-04-06 11:25:29.816166820.txt',
        '2018-04-06 11:25:29.816166820~2018-04-06 11:36:07.846152818.txt',
        '2018-04-06 11:36:07.846152818~2018-04-06 11:46:36.696140077.txt',
        '2018-04-06 11:46:36.696140077~2018-04-06 11:56:46.836124689.txt',
        '2018-04-06 11:56:46.836124689~2018-04-06 12:06:51.746122925.txt',
        '2018-04-06 12:06:51.746122925~2018-04-06 14:01:32.489584227.txt'
    ]
...
def calc_attack_edges():
    ...
    for f in attack_list:
        files.append(f"{artifact_dir}/graph_4_6_10min/{f}")
...
if __name__ == "__main__":
    logger.info("Start logging. adversarial 160")

    # Validation date
    anomalous_queue_scores = []
    history_list = torch.load(f"{artifact_dir}/graph_4_5_history_list_10min")
    ...
    history_list = torch.load(f"{artifact_dir}/graph_4_6_history_list_10min_adversarial_160")
    for hl in history_list:
        anomaly_score = 0
        for hq in hl:
            anomaly_score += math.log(hq['loss'] + 1)
        name_list = [i['name'] for i in hl]
        print(anomaly_score)
        if anomaly_score > beta_day6:  # 阈值需重新用验证集分布确定
            logger.info(f"Anomalous queue: {name_list}")
            for i in name_list:
                pred_label[i] = 1
            logger.info(f"Anomaly score: {anomaly_score}")

    history_list = torch.load(f"{artifact_dir}/graph_4_7_history_list_10min")
    for hl in history_list:
        anomaly_score = 0
        for hq in hl:
            anomaly_score += math.log(hq['loss'] + 1)
        name_list = [i['name'] for i in hl]
        print(anomaly_score)
        if anomaly_score > beta_day7:  # 阈值需重新用验证集分布确定
            logger.info(f"Anomalous queue: {name_list}")
            for i in name_list:
                pred_label[i] = 1
            logger.info(f"Anomaly score: {anomaly_score}")
