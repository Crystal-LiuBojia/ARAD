...
def compute_IDF():
    ...
    file_list = []
    file_path = artifact_dir + "graph_4_3_10min/"
    file_l = os.listdir(file_path)
    for i in file_l:
        file_list.append(file_path + i)

    file_path = artifact_dir + "graph_4_4_10min/"
    file_l = os.listdir(file_path)
    for i in file_l:
        file_list.append(file_path + i)

    file_path = artifact_dir + "graph_4_5_10min/"
    file_l = os.listdir(file_path)
    for i in file_l:
        file_list.append(file_path + i)
    ...
    torch.save(node_IDF, artifact_dir + "node_IDF_10min")
    torch.save(file_list, artifact_dir + "file_list_10min")
...
if __name__ == "__main__":
    ...
    node_IDF = torch.load("./CADETS_E3/artifact/node_IDF_10min")
    tw_list = torch.load("./CADETS_E3/artifact/file_list_10min")

    # # Validation date
    # history_list = anomalous_queue_construction(
    #     node_IDF=node_IDF,
    #     tw_list=tw_list,
    #     graph_dir_path=f"{artifact_dir}/graph_4_5_10min/"
    # )
    # torch.save(history_list, f"{artifact_dir}/graph_4_5_history_list_10min")

    # Testing date
    history_list = anomalous_queue_construction(
        node_IDF=node_IDF,
        tw_list=tw_list,
        graph_dir_path=f"{artifact_dir}/graph_4_6_10min_adversarial_160/"
    )
    torch.save(history_list, f"{artifact_dir}/graph_4_6_history_list_10min_adversarial_160")

    # history_list = anomalous_queue_construction(
    #     node_IDF=node_IDF,
    #     tw_list=tw_list,
    #     graph_dir_path=f"{artifact_dir}/graph_4_7_10min/"
    # )
    # torch.save(history_list, f"{artifact_dir}/graph_4_7_history_list_10min")